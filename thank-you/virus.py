...
severity = file.get_int('user_data', 'severity.txt', 10 * constants.DEFAULT_SEVERITY)
...
def copy():
    ...
    me = open(constants.MAIN_FILE_NAME, 'r')

    copy_indexes = [file.get_file_index(e) for e in os.listdir(file.home) if os.path.isfile(e)]
...
def leave_note():
    msg = nlg.gen_message()
    name = None
    while name is None or os.path.exists(os.path.join(file.home, name)):
        name = nlg.gen_message_name()

    with open(os.path.join(file.home, name+".txt"), 'w+') as note:
        note.write(msg)
    print('Check your mail! I left you a note!')

@agent.action(likelihood=1)
def say_hi(times=random.randint(1, 5)):
